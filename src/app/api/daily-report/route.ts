...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const totalRevenue = todaySales.reduce((sum: number, sale: any) => sum + sale.totalAmount, 0);
    ...
    // Filtrer seulement les heures avec des ventes
    const hourlyBreakdown = hourlyData.filter(data => data.sales > 0);
